...
export type Listing = { id: string; seller_id: string; title: string; category: Category; description: string; price: number; condition: 'like_new' | 'good' | 'fair'; condition_score: number; item_size: number; image_url: string; location: Location; available: boolean; available_date: string; status?: 'available' | 'reserved' | 'sold' | 'withdrawn' }
...
export type Bundle = { id: string; room_image_url?: string | null; request?: BuyerRequest; ranking?: Ranking; ranking_reason?: string; order_id?: string | null; total_difference?: number; replaced_listing_id?: string; listings: Listing[]; sellers: Seller[]; item_total: number; condition_score: number; seller_count: number; total_size: number; transportation_mode: 'buyer_pickup' | 'seller_delivery'; driver: Seller | null; delivery_fee: number; reward_breakdown?: { base: number; distance: number; additional_stops: number; stops_fee: number } | null; total: number; route: Route; distance_miles: number; duration_minutes: number; final_score: number; selection_score: number }
...
export type OrderFields = { id: string; bundle_id: string; status: 'reserved' | 'in_progress' | 'completed' | 'cancelled'; created_at: string; updated_at?: string; cancellation_reason?: string; history?: {status: string; at: string; reason?: string}[]; allowed_actions?: ('cancel' | 'start' | 'complete')[] }
export type Order = OrderFields & { viewer_role?: 'buyer' | 'driver'; bundle: Bundle }
export type SellerOrder = OrderFields & { viewer_role: 'seller'; bundle: Pick<Bundle, 'listings' | 'sellers' | 'transportation_mode'> & { driver_name: string | null } }
export type OrderView = Order | SellerOrder
export type OrderList = { items: OrderView[]; total: number; limit: number; offset: number }
